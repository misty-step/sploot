...
- [x] Add a cron-protected `/api/cron/purge-search-logs` route that deletes `search_logs` older than 30 days using timing-safe auth; Success criteria: route reports deleted row count, uses `crypto.timingSafeEqual`, and completes in a single delete statement.
- [x] Tighten `/api/assets` list response to a minimal field projection (path, blobUrl, mime, dims, size, favorite, createdAt) and gate tag expansion behind an explicit query flag; Success criteria: default list page response for 50 assets <10KB and tag data still available when requested.
