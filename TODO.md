...
- [ ] Refactor `hooks/use-status-stats.ts` to consume `/api/stats` instead of `/api/assets?limit=1000`, and slow polling to ~30s idle / 5s when queue active; Success criteria: status bar still renders counts, no `/api/assets?limit=1000` calls during polling, network payload per poll stays sub‑1KB.
