...
test('live usage reconciles without identifiers or silently-zero unknowns', async () => {
  ...
  assert.equal(inputs.liveUsage.database.databaseBytes, 58_245_120);
  assert.equal(inputs.liveUsage.inference.latestPredictionSample.failed, 52);
  assert.equal(inputs.liveUsage.telemetry.errors30d, 8_411);
  assert.equal(inputs.liveUsage.digitalOcean.monthToDateUsageUsd, 113.75);
  assert.equal(inputs.liveUsage.digitalOcean.invoicePreviewUsd, 109.22);
  assert.equal(inputs.liveUsage.digitalOcean.namedVarianceUsd, 4.53);
