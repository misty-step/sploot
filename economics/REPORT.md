...
- Vercel Blob: 6,568 objects / 536.7 MB versus 508.1 MB live plus 0.019 MB deleted source bytes in Postgres (508.2 MB total). The derived gap is 28,506,462 bytes (5.61% of live-plus-deleted source bytes). The displayed gap is derived from Blob bytes minus live-plus-deleted Postgres source bytes; the residual may include thumbnails or unreferenced objects. Vercel Blob billing remains account-wide and unattributed at store level.
- Neon/Postgres: 58.2 MB database, 10 users, 3,140 ready embeddings.
...
- DigitalOcean: invoice preview $109.22 versus account month-to-date usage $113.75, a named $4.53 variance. The 2026-07-25 redacted account readback is $113.75 month-to-date versus a $109.22 invoice preview, a named $4.53 cadence/account-wide variance. Preview and balance endpoints update on different cadences and cover the entire account, which hosts multiple apps. DigitalOcean exposes no per-app accrued-transfer or invoice-preview allocation, so Sploot's exact July line item is not inferable from this authority.
...
- Vercel project (2026-07-01..2026-07-25): effective usage $0.0000000000, including project-attributed platform usage $0.000000, plus a $0.0000000000 aggregate remainder where each remaining category is below $0.01. The misty-step Vercel scope contains no Sploot project: the web runtime migrated to DigitalOcean App Platform (ADR-009), so project-attributed Vercel platform usage is zero for this period. Vercel Blob remains the only Vercel surface in use and is billed account-wide. Vercel Blob remains account-wide and unattributed at store level.
...
- Canary: 8,411 Sploot errors in 30 days. Canary reports 8,411 Sploot errors across 50 unique classes in the last 30 days, read from its aggregate query API; the most frequent class is EmbeddingAdmissionError. Canary is a self-hosted shared DigitalOcean component; this is event usage, not a separate vendor bill, so the model allocates fixed component cost instead of inventing a per-event price.
