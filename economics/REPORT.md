...
- Vercel Blob: 6,557 objects / 534.1 MB versus 501.5 MB live plus 0.019 MB deleted source bytes in Postgres (501.5 MB total). The derived gap is 32,547,032 bytes (6.49% of live-plus-deleted source bytes). The displayed gap is derived from Blob bytes minus live-plus-deleted Postgres source bytes; the residual may include thumbnails or unreferenced objects. Vercel Blob billing remains account-wide and unattributed at store level.
- Neon/Postgres: 42.2 MB database, 10 users, 3,136 ready embeddings.
- Replicate: latest 100 predictions were 77 failed, 1 canceled, and 22 succeeded. This is operational usage, not a bill. Public-model failed predictions are documented as unbilled, while canceled/time-based work may bill; exact Replicate dollars are unavailable to the API authority.
- DigitalOcean: invoice preview $56.88 versus account month-to-date usage $62.27, a named $5.39 variance. The 2026-07-17 redacted account readback is $62.27 month-to-date versus a $56.88 invoice preview, a named $5.39 cadence/account-wide variance. Preview and balance endpoints update on different cadences and cover the entire account, which hosts multiple apps. DigitalOcean exposes no per-app accrued-transfer or invoice-preview allocation, so Sploot's exact July line item is not inferable from this authority.
...
- Vercel project (2026-07-01..2026-07-17): effective usage $1.6836573508, including build CPU $0.778009, analytics $0.459150, observability $0.222658, provisioned memory $0.067224, active CPU $0.064374, origin transfer $0.044212, invocations $0.039175, plus a $0.0088553508 aggregate remainder where each remaining category is below $0.01. Sploot project effective usage for the current period is attributed by project. The remaining categories are each below $0.01 and are retained as an aggregate sub-cent remainder; the provider readback describes usage as almost entirely absorbed by allowance and billed usage as almost $0, but does not provide exact post-allowance dollars. Vercel Blob remains account-wide and unattributed at store level.
- Modeled known-cost reconciliation: current web, embedding-job schedule, Blob bytes at the on-demand rate, and database bytes at the Launch storage rate produce a $28.36 monthly baseline. It deliberately excludes unknown history/WAL, operations, and transfer rather than treating them as zero. The $28.52 difference to the account-wide invoice preview is deliberately not attributed to Sploot: it contains unrelated apps, Canary allocation, other jobs, transfer/operations, and endpoint timing.
- Canary: 14,683 Sploot errors in 30 days. The 2026-07-17 readback came directly from Canary's authoritative production SQLite store while its public read API was unavailable. Canary is a self-hosted shared DigitalOcean component; this is event usage, not a separate vendor bill, so the model allocates fixed component cost instead of inventing a per-event price.
- GitHub: public repository, 14 active caches / 5.62 GiB. 5.62 GiB is below the separate 10 GiB per-repository cache allowance; standard public-repository runners are free.
