...
- [ ] Complete a provider decision spike comparing Clerk-wrapped, Better Auth,
      Supabase Auth, Auth.js/custom, and Keycloak/managed Keycloak against
      agent-readiness criteria.
- [ ] Add a typed `AuthenticatedPrincipal`, `AuthPolicy`, and auth result model.
- [ ] Add one route-facing auth wrapper that hides Clerk cookie auth, bearer
      auth, optional user sync, and JSON unauthorized response shape.
- [ ] Migrate at least one representative protected route to the wrapper while
      preserving `401 {"error":"Unauthorized"}`.
- [ ] Add a deterministic local/CI QA auth mode that is disabled in production.
- [ ] Add Playwright authenticated web smoke covering `/app` without manual
      Clerk login.
- [ ] Add extension API-client token-provider injection so tests can pass a
      deterministic token provider.
- [ ] Add docs for auth modes, required env vars, and which modes are permitted
      in local, CI, preview, production, and release proof.
- [ ] Preserve secret-scanning and do not commit `.auth` storage state or real
      Clerk credentials.
