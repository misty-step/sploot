...
Use /flywheel for repeated Sploot delivery cycles: pick -> /shape -> /implement -> /yeet -> /settle -> /ship -> /monitor -> loop. /ship owns closure and /reflect; /flywheel consumes the result and selects the next active risk.

Cycle selection should prefer operational product risk: save/search/shuffle correctness, upload/auth/storage quota, pgvector/search health, extension release readiness, Sentry incidents, release automation, and API docs drift. Stop the loop when the next item needs user credentials, Web Store dashboard action, or product decision.
