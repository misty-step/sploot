...
Use /diagnose for failed gates, production symptoms, release blockers, or confusing extension/auth behavior. Start with the exact failing command/artifact, then trace source-to-sink through the owning surface.

Common Sploot diagnostic paths: Prisma/pgvector auth and migrations, stale serverless DB connections in health routes, Clerk authorized parties and extension origins, background upload logs, Vercel Blob upload responses, Replicate embedding jobs/rate limits, semantic-release `GH_RELEASE_TOKEN`, and mismatches between API routes and `apps/web/docs/API.md`. After two repeated failures, stop and change strategy rather than retrying.
