...
Use /ci to run or diagnose the CI parity gate. Local baseline: `pnpm lint && pnpm type-check && pnpm --filter web test && pnpm --filter extension build`. For true CI parity on DB-backed paths, first run `pnpm --filter web db:migrate` and web tests with `DATABASE_URL` pointing at pgvector Postgres.

Remote CI also runs frozen install, extension lint/test/build, and the aggregate `merge-gate`. Lefthook runs gitleaks, web/extension lint, and turbo type-check before local commit/push. Do not lower gates; diagnose env, migrations, ports, pnpm cache, WXT build output, or test data instead.
