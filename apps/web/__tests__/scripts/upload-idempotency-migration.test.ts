...
describe('upload idempotency migration posture', () => {
  it('is transactional, indexed for bounded cleanup, and has an explicit rollback/readback contract', () => {
    ...
    const appSelectGrant = restrictedGrant.match(/GRANT SELECT, INSERT, UPDATE, DELETE ON TABLE([\s\S]*?)TO sploot_stripe_app;/)?.[1] ?? '';
    expect(appSelectGrant.replace(/\s+/g, ' ')).toContain('public.upload_idempotency');
  });
});
