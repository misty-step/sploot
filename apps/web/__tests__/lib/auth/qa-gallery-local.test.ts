...
} from '@/lib/auth/qa-gallery-local';
...
describe('cross-seam binding isolation', () => {
  it('rejects a PWA-bound token presented to the Gallery adapter', async () => {
    const pwa = await import('@/lib/auth/qa-local');
    const secret = 'shared-test-secret-with-enough-entropy';
    const pwaToken = await pwa.createQaLocalAuthToken({
      userId: 'qa-user-1',
      secret,
      expiresInSeconds: 60,
    });
    const result = await verifyQaLocalAuthHeaders(qaRequest(pwaToken), QA_ENV, { host: 'localhost', remoteAddress: '127.0.0.1' });
    expect(result.status).not.toBe('authenticated');
  });

  it('rejects a Gallery-bound token presented to the PWA adapter', async () => {
    const pwa = await import('@/lib/auth/qa-local');
    const secret = 'shared-test-secret-with-enough-entropy';
    const galleryToken = await createQaLocalAuthToken({
      userId: 'qa-user-1',
      secret,
      expiresInSeconds: 60,
    });
    const headers = new Headers({ [pwa.getQaLocalAuthHeader()]: galleryToken });
    const pwaEnv = {
      NODE_ENV: 'test',
      SPLOOT_DEPLOYMENT_ENV: 'test',
      SPLOOT_QA_AUTH_MODE: 'enabled',
      SPLOOT_QA_AUTH_SECRET: secret,
      SPLOOT_QA_DEPLOYMENT_ID: pwa.QA_LOCAL_DEPLOYMENT_ID,
      SPLOOT_QA_DEPLOYMENT_ENV: pwa.QA_LOCAL_DEPLOYMENT_ENV,
      SPLOOT_QA_AUDIENCE: pwa.QA_LOCAL_AUDIENCE,
    };
    const result = await pwa.verifyQaLocalAuthHeaders(headers, pwaEnv, { peerAddress: '127.0.0.1' });
    expect(result.status).not.toBe('authenticated');
  });
});
