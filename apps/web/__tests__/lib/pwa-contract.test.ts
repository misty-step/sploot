...
describe('PWA install/share-target contract', () => {
  ...
  it('rejects protected /app documents planted into the precache manifest', () => {
    const source = readFileSync(serviceWorkerFixture, 'utf8');
    const tempDir = mkdtempSync(join(webRoot, '.pwa-contract-'));
    try {
      for (const protectedPath of ['/app', '/app/library']) {
        const mutated = source.replace("/manifest.json', revision: 'fixture'", `${protectedPath}', revision: 'fixture'`);
        const serviceWorkerPath = join(tempDir, `${protectedPath.slice(1).replace('/', '-')}.js`);
        writeFileSync(serviceWorkerPath, mutated);
        expect(() => execFileSync(process.execPath, [script], {
          cwd: webRoot,
          env: { ...validatorEnv, PWA_SERVICE_WORKER_FILE: serviceWorkerPath },
          stdio: 'pipe',
  ...
  it('rejects a handler that only mentions NetworkOnly and a cached NetworkOnly route', () => {
    const source = readFileSync(serviceWorkerFixture, 'utf8');
    const tempDir = mkdtempSync(join(webRoot, '.pwa-contract-'));
    try {
      const nestedStrategy = source.replace(
        'new workbox.NetworkOnly()',
        'new workbox.CacheFirst({ plugins: [workbox.NetworkOnly] })',
      );
      const nestedPath = join(tempDir, 'nested-strategy.js');
      writeFileSync(nestedPath, nestedStrategy);
      expect(() => execFileSync(process.execPath, [script], {
        cwd: webRoot,
        env: { ...validatorEnv, PWA_SERVICE_WORKER_FILE: nestedPath },
        stdio: 'pipe',
      })).toThrow();

      const cachedStrategy = source.replace(
        'new workbox.NetworkOnly()',
        "new workbox.NetworkOnly({ cacheName: 'protected-documents' })",
      );
      const cachedPath = join(tempDir, 'cached-strategy.js');
      writeFileSync(cachedPath, cachedStrategy);
      expect(() => execFileSync(process.execPath, [script], {
        cwd: webRoot,
        env: { ...validatorEnv, PWA_SERVICE_WORKER_FILE: cachedPath },
        stdio: 'pipe',
      })).toThrow();
    } finally {
      rmSync(tempDir, { recursive: true, force: true });
