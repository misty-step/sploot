...
function hasPropertyNamed(node, name) {
  ...
  const visit = (candidate) => {
    if (found) return;
    if (ts.isPropertyAccessExpression(candidate) && candidate.name.text === name) {
      found = true;
      return;
    }
...
function hasObjectPropertyNamed(node, name) {
  let found = false;
  const visit = (candidate) => {
    if (found) return;
    if ((ts.isPropertyAssignment(candidate) || ts.isShorthandPropertyAssignment(candidate)) && propertyName(candidate) === name) {
      found = true;
      return;
    }
    ts.forEachChild(candidate, visit);
...
function hasIdentifierNamed(node, name) {
  let found = false;
  const visit = (candidate) => {
    if (found) return;
...
function hasPropertyComparison(node, property, expected) {
  let found = false;
  const visit = (candidate) => {
    if (found) return;
    if (ts.isBinaryExpression(candidate) && [
      ...
      const left = staticString(candidate.left);
      const right = staticString(candidate.right);
      if ((left === expected && hasPropertyNamed(candidate.right, property)) || (right === expected && hasPropertyNamed(candidate.left, property))) {
        found = true;
        return;
      }
    }
...
function hasMethodCall(node, method, argument) {
  let found = false;
  const visit = (candidate) => {
    if (found) return;
    if (ts.isCallExpression(candidate) && ts.isPropertyAccessExpression(candidate.expression) && candidate.expression.name.text === method && staticString(candidate.arguments[0]) === argument) {
      found = true;
      return;
    }
    ts.forEachChild(candidate, visit);
...
function findNamedCalls(sourceFile, name) {
  const calls = [];
  const visit = (node) => {
    if (isNamedCall(node, name)) calls.push(node);
...
function readPrecacheUrls(call) {
  const manifest = call.arguments[0];
  if (!manifest || !ts.isArrayLiteralExpression(manifest) || manifest.elements.length === 0) return null;
  const urls = [];
  for (const element of manifest.elements) {
    if (!ts.isObjectLiteralExpression(element)) return null;
    const urlProperty = element.properties.find((property) => {
      if (!ts.isPropertyAssignment(property)) return false;
      return propertyName(property) === 'url';
    });
    if (!urlProperty || !ts.isPropertyAssignment(urlProperty)) return null;
    const url = staticString(urlProperty.initializer);
...
function inspectGeneratedWorker(source) {
  const sourceFile = ts.createSourceFile('sw.js', source, ts.ScriptTarget.Latest, true, ts.ScriptKind.JS);
  const hasInstallCall = (name) => findNamedCalls(sourceFile, name).length > 0;
  if (!hasInstallCall('skipWaiting') || !hasInstallCall('clientsClaim')) {
    fail('service worker install/update contract is missing skipWaiting or clientsClaim');
  }
  const precacheCall = findNamedCalls(sourceFile, 'precacheAndRoute')[0];
  const precacheUrls = precacheCall ? readPrecacheUrls(precacheCall) : null;
  if (!precacheUrls) fail('service worker precache manifest is missing or not statically inspectable');
  if (precacheUrls.some((path) => path === '/app' || path.startsWith('/app/'))) {
    fail('service worker must not precache protected /app documents');
  }
  const protectedRoute = findNamedCalls(sourceFile, 'registerRoute').find((call) => {
    const matcher = call.arguments[0];
    const handler = call.arguments[1];
    return matcher && isNetworkOnlyHandler(handler)
      && hasPropertyComparison(matcher, 'mode', 'navigate')
      && hasPropertyComparison(matcher, 'pathname', '/app')
      && hasMethodCall(matcher, 'startsWith', '/app/')
      && !hasObjectPropertyNamed(handler, 'cacheName');
  });
  if (!protectedRoute) fail('service worker protected /app navigation must be network-authoritative without a cache');
  return precacheUrls;
}

async function validateServiceWorker() {
  const source = (await readAsset(SERVICE_WORKER_FILE, 'service worker')).toString('utf8');
  inspectGeneratedWorker(source);
